...
type ScorePoint = { date: string; score: number; passed: boolean };

type TooltipPayload = { value: number; payload: ScorePoint };
...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: TooltipPayload[]; label?: string }) {
  ...
  return (
    <div
      ...
      style={{ background: "#1F2937", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 4px 20px rgba(0,0,0,0.5)" }}
    >
...
export function ScoreLineChart({ data }: Props) {
  if (data.length === 0) {
    return (
      ...
      >
        <p className="text-white/25 text-sm">Sem dados de evolução ainda.</p>
      </div>
    );
  }
      ...
      <div className="px-2 py-4">
        <ResponsiveContainer width="100%" height={160}>
          <LineChart data={data} margin={{ top: 8, right: 16, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="date" tick={{ fontSize: 9, fill: "rgba(255,255,255,0.2)" }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
            <YAxis domain={[0, 100]} tick={{ fontSize: 9, fill: "rgba(255,255,255,0.2)" }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}%`} />
            ...
            <Line
              ...
              strokeWidth={2}
              dot={(props) => {
                const { cx, cy, payload } = props as { cx: number; cy: number; payload: ScorePoint };
                return (
                  <circle
                    key={`dot-${cx}-${cy}`}
                    cx={cx} cy={cy} r={3}
